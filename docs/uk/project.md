...
4. **Запобігання "білому спалаху" (Anti-Flash White Pattern):**
   - Для найкращого User Experience у кожній деталі, HTML сторінки, що підтримують світлу та темну теми, ПОВИННІ мати темну тему за замовчуванням (або хоча б темний фон).
   - Це реалізується прямо у вихідному HTML файлі (наприклад, `index.html`) через встановлення атрибуту `data-bs-theme="dark"` на тегу `<html>` та інлайн-стилів для `<body background-color="...">` ще до завантаження JS чи зовнішніх CSS.
   - Це гарантує, що у нічний час користувач не отримає сліпучого спалаху світла навіть на півсекунди.
   - Коментарі в HTML/CSS повинні бути англомовними для багатомовних продуктів.
   
   *Приклад архітектурного стандарту `index.html`:*
   ```html
   <!doctype html>
